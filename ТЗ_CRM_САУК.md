...
|------|-----|----------|
| `id` | PK | |
| `inn` | CharField(12), unique | ИНН организации или ИП |
| `ogrn` | CharField(15), nullable | ОГРН |
| `kpp` | CharField(9), nullable | КПП |
| `full_name` | CharField(500) | Полное юридическое наименование |
| `short_name` | CharField(200) | Сокращённое наименование |
| `legal_address` | TextField, nullable | Юридический адрес (из реестра) |
| `client_type` | CharField | `organization` / `individual` / `entrepreneur` |
| `is_active` | Boolean | Активный клиент |
...
> **Примечание реализации:** API DaData требует токена. Как fallback — парсинг ЕГРЮЛ через официальный сервис egrul.nalog.ru. Реализовать через AJAX: поле ИНН + кнопка «Найти».
...
| `position` | CharField(200), nullable | Должность |
| `email` | EmailField, nullable | Email |
| `telegram_username` | CharField(100), nullable | @username в Telegram |
| `telegram_id` | BigIntegerField, nullable | Числовой ID Telegram |
...
| `id` | PK | |
| `client` | FK → Client | |
| `address_type` | CharField | `legal` / `actual` / `delivery` / `other` |
| `address` | TextField | Полный адрес |
| `description` | TextField, nullable | Комментарий к адресу |
| `is_primary` | Boolean | Основной адрес |
